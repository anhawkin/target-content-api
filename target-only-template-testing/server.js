...
const app = express();
var TEMPLATE = fs.readFileSync(__dirname + "/templates/index-simple.tpl").toString();
...
function sendHtml(res, offer) {
    if (offer.content != undefined && offer.content == 'B')
        {
            TEMPLATE = fs.readFileSync(__dirname + "/templates/index.tpl").toString();   
        }
    else {
        TEMPLATE = fs.readFileSync(__dirname + "/templates/index-simple.tpl").toString();
}
...
app.get("/", function (req, res) {
  ...
  const payload = {"mbox" : "a1-serverside-template"};
